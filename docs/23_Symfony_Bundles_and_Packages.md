...
1. Исправляем секцию `autoload` в `composer.json`
    ...
    ```
1. Переносим класс `App\Client\StatsdAPIClient` в пространство имён `StatsdBundle\Client`:
...
1. Подключаем наш бандл в файле `config/bundles.php`
    ...
    ```
1. В классе `App\Controller\Api\SaveUser\v5\SaveUserManager` исправляем метод `saveUser`
    ```php
...
1. Выполняем запрос Add user v5 из Postman-коллекции v10 и проверяем, что данные поступают в Graphite
...
1. Выполняем ещё один запрос Add user v5 из Postman-коллекции v10 и проверяем, что данные поступают в Graphite

## Выносим бандл в отдельный репозиторий

1. Создаём новый репозиторий в git и переносим в него содержимое 'src/StatsdBundle'
1. В файле `config/services.yaml` в секции `services.App\` убираем исключение `../src/StatsdBundle/`
...
1. Выполняем ещё один запрос Add user v5 из Postman-коллекции v10 и проверяем, что данные всё ещё поступают в Graphite
...
1. Подключаем наш бандл в файле `config/bundles.php`
    ...
    return [
        ...
        Nelmio\ApiDocBundle\NelmioApiDocBundle::class => ['all' => true],
        StatsdBundle\StatsdBundle::class => ['all' => true],
        FeeddBundle\FeedBundle::class => ['all' => true],
    ];
    ```
1. В файле `config/services.yaml`
    1. в секции `services.App\` добавляем исключение:
...
1. Создаём класс `FeedBundle\DTO\TweetDTO`
    ...
    ```
1. Переносим в пространство имён `FeedBundle` и исправляем класс `App\Consumer\UpdateFeed\Input\Message`
    ```php
    <?php
...
1. Переносим в пространство имён `FeedBundle` и исправляем класс `App\Service\FeedService`
    ...
    ```
1. Переносим в пространство имён `FeedBundle` и исправляем класс `App\Consumer\UpdateFeed\Consumer`
    ```php
    <?php
...
1. Добавляем 10 подписчиков командой `php bin/console followers:add 1 -lnew_follower`
1. Перезапускаем supervisor командой `docker-compose restart supervisor`
1. Выполняем запрос Post tweet v5 из Postman-коллекции v10 и проверяем, что 
     1. данные поступают в Graphite
     1. отправляются сообщения в очереди с префиксом `send_notification`
1. Выполняем запрос Get feed из Postman-коллекции v10 и видим, что лента возвращается
