...
1. В классе `App\Repository\TweetRepository` добавляем метод `getByAuthorIds`
    ...
    ```
1. В классе `App\Manager\TweetManager` добавляем метод `getFeed`
    ```php
    /**
     * @param int[] $authorIds
     *
    ...
    ```
1. В классе `App\Manager\SubscriptionManager` добавляем метод `findAllByFollower`
    ```php
    /**
     * @return Subscription[]
     */
...
1. Перезапускаем контейнеры
    ...
    ```
1. Заходим в контейнер командой `docker exec -it php sh`. Дальнейшие команды выполняем из контейнера
1. Применяем миграции к тестовой базу командой `php bin/console doctrine:migrations:migrate --env=test`
