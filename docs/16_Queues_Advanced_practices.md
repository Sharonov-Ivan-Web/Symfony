...
1. Создаём миграцию и применяем её командами
    ```shell
...
1. В классе `App\Entity\Tweet` добавляем два новых метода:
    ...
    ```
1. Добавляем класс `App\Service\FeedService`
    ```php
    <?php
    ...
    ```
1. Добавляем класс `App\Controller\Api\GetFeed\v1\Controller`
    ```php
    <?php
    ...
    ```
1. В классе `App\Manager\TweetManager` исправляем метод `saveTweet`
    ```php
    /**
...
1. Исправляем класс `App\DTO\SaveUserDTO`
    ...
    ```
1. В классе `App\Manager\UserManager` исправляем метод `saveUserFromDTO`
    ```php
...
1. Выполняем запрос Add followers из Postman-коллекции v8 с параметрами `async` = 1 и `count` = 1000, проверяем, что
...
1. Добавляем класс `App\Manager\EmailNotificationManager`
    ...
    ```
1. Добавляем класс `App\Manager\SmsNotificationManager`
    ```php
    <?php
...
1. Добавляем класс `App\DTO\SendNotificationDTO`
    ...
    ```
1. В класс `App\Manager\SubscriptionManager` добавляем метод `findAllByAuthor`
    ```php
    /**
     * @return Subscription[]
     */
...
1. Добавляем описание новых продюсеров и консьюмеров в файл `config/packages/old_sound_rabbit_mq.yaml`
    ...
    1. в секцию `consumers`
         ...
         send_notification.sms:
             ...
             qos_options: {prefetch_size: 0, prefetch_count: 1, global: false}
         ```
1. Запускаем все новые консьюмеры командами
     ```shell
     ...
     ```
1. Выполняем запрос Post tweet из Postman-коллекции v8 с параметром `async` = 1
...
1. Выполняем несколько запросов Post tweet из Postman-коллекции v8 с параметром `async` = 1, проверяем, что консьюмеры
...
`App\Consumer\UpdateFeed\ConsumerK`
    ...
    ```
1. Перезапускаем контейнер `supervisor` командой `docker-compose restart supervisor`
1. Выполняем несколько запросов Post tweet из Postman-коллекции v8 с параметром `async` = 1
